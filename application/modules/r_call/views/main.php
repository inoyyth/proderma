                    ...
                    <div class="row">
                        <form class="form-filter-table">
                            ...
                            <div class="col-lg-2">
                                <div class="form-group">
                                    <label class="small">&nbsp;</label>
                                    <button type="button" style="margin-top: 24px;" onclick="subReport(1);" class="btn btn-xs btn-primary btn-search">Search</button>
                                </div>
                            </div>
                            <div class="col-lg-2">
                                <div class="form-group">
                            ...
                            <div class="col-lg-2">
                                <div class="form-group">
                                    <label class="small">&nbsp;</label>
                                    <button type="button" style="margin-top: 24px;" onclick="subReport(2);" class="btn btn-xs btn-primary btn-search">Search</button>
                                </div>
                            </div>
                        </form>
                    </div>
...
function subReport(tp) {
    ...
    $.ajax({
        type: "POST",
        url: '<?php echo base_url('r_call/getReport'); ?>',
        data: dt,
    ...
    .done(function (data) {
        ...
        $("#example-table").tabulator({
            ...
            fitColumns:true, //fit columns to width of table (optional),
                    ajaxType: "POST", //ajax HTTP request type
            ajaxURL: "<?php echo base_url('r_call/getListTable'); ?>", //ajax URL
            ajaxParams: tabulatorAjaxParams,
            groupBy: "kelompok",
            columns: [//Define Table Columns
                {formatter: "rownum", align: "center", width: 40},
                {title: "Code", field: "order_id", tooltip: true},
                {title: "Customer Code", field: "customer_code"},
                {title: "Customer Name", field: "customer_name"},
                {title: "Visit Date", field: "sales_visit_date"},
                {title: "End Date", field: "end_date"},
                {title: "Activity", field: "objective"},
                {title: "Status", field: "sales_visit_progress"}
            ],
            selectable: 1,
        });
    });
